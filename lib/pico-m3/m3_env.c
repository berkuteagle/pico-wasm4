...
#if M3_STATIC_ENV
static M3Environment environment;
#endif

IM3Environment m3_NewEnvironment()
{
#if M3_STATIC_ENV
    IM3Environment env = &environment;
#else
    ...
    if (env)
    {
        _try
        {
            ...
            for (uint8_t t = c_m3Type_none; t <= c_m3Type_f64; t++)
            {
                IM3FuncType ftype;
...
M3Result EvaluateExpression(IM3Module i_module, void *o_expressed, uint8_t i_type, bytes_t *io_bytes, cbytes_t i_end)
{
...
M3Result InitDataSegments(M3Memory *io_memory, IM3Module io_module)
{
    ...
    for (u32 i = 0; i < io_module->numDataSegments; ++i)
    {
        ...
        {
            uint8_t *dest = m3MemData(io_memory->mallocated) + segmentOffset;
...
uint8_t *GetStackPointerForArgs(IM3Function i_function)
{
