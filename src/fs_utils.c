...
static int littlefs_sync(const struct lfs_config *c) { return 0; }
...
int fs_read_file(const char *path, uint8_t *buffer, uint32_t buffer_size,
...
int fs_mkdir(const char *path) {
  ...
  if (lfs_stat(&fs_external, path, &info) == LFS_ERR_NOENT) {
    if (lfs_mkdir(&fs_external, path)) {
      return -1;
    }
  }
...
const struct lfs_config *fs_get_lfs_config() { return &fs_external_config; }
