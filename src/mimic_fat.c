...
static uint8_t fat_boot_sector[] = {
    ...
    0x00, 0x10,                                                // BPB_TotSec16
    0xF8,                                                      // BPB_Media
    0x10, 0x00,                                                // BPB_FATSz16
    ...
    'P',  'i',  'c',  'o',  ' ', 'W', 'A', 'S', 'M', '4', ' ', // BS_VolLab
    'F',  'A',  'T',  '1',  '6', ' ', ' ', ' '                 // BS_FilSysType
};

#define FAT_BOOT_SECTOR_SIZE (sizeof(fat_boot_sector))

static const mimic_file_t files[] = {
    {
        .name = "cart.wasm",
        .path = "/cart.wasm",
        .exists = false,
...
#define FILES_COUNT (sizeof(files) / sizeof(mimic_file_t))

static uint8_t file_entry[] = {
    'F',  'I',  'L',  'E', 'N', 'A', 'M', 'E', // Name
    'E',  'X',  'T',                           // Ext
    0x00,                                      // Attrs
    0x00,                                      // Reserved
    0x00,                                      // CreateTimeFine
    0x00, 0x00,                                // CreateTime
    0x00, 0x00,                                // CreateDate
    0x00, 0x00,                                // LastAccessDate
    0x00, 0x00,                                // HI StartCluster
    0x6d, 0x4f,                                // UpdateTime
    0x43, 0x65,                                // UpdateDate
    0x00, 0x00,                                // LO StartCluster
    0x00, 0x00, 0x00, 0x00                     // Size
};

#define FILE_ENTRY_SIZE (sizeof(file_entry))

static const char *disk_label = "Pico2 WASM4";
...
void mimic_fat_init() { littlefs_lfs_config = fs_get_lfs_config(); }

bool mimic_fat_usb_device_is_enabled(void) { return usb_device_is_enabled; }
  ...
  for (size_t i = 0; i < DISK_SECTOR_SIZE / sizeof(fat_dir_entry_t); i++) {
    ...
    if ((dir->DIR_Attr & 0x0F) != 0x0F) {
      ...
      TRACE("name='%s' attr=0x%02X size=%lu cluster=%u\n", pbuffer,
            dir->DIR_Attr, dir->DIR_FileSize, dir->DIR_FstClusLO);
    } else {
      ...
      TRACE("name='%s' attr=0x%02X ord=0x%02X checksum=%u  cluster=%u\n", utf8,
            lfn->LDIR_Attr, lfn->LDIR_Ord, lfn->LDIR_Chksum,
            dir->DIR_FstClusLO);
    }
    dir++;
  }
...
static uint16_t read_fat(int cluster) {
  ...
  lfs_soff_t o =
      lfs_file_seek(&real_filesystem, &fat_cache, offset, LFS_SEEK_SET);
  if (o < 0) {
  ...
  uint8_t current[2] = {0};
  lfs_ssize_t s =
      lfs_file_read(&real_filesystem, &fat_cache, current, sizeof(current));
  if (s < 0) {
...
static void update_fat(uint32_t cluster, uint16_t value) {
  ...
  lfs_ssize_t s =
      lfs_file_read(&real_filesystem, &fat_cache, previous, sizeof(previous));
  if (s < 0) {
...
static size_t bulk_update_fat_buffer_read(size_t offset, void *buffer,
                                          size_t size) {
  lfs_soff_t o =
      lfs_file_seek(&real_filesystem, &fat_cache, offset, LFS_SEEK_SET);
  if (o < 0) {
...
static size_t bulk_update_fat_buffer_write(size_t offset, void *buffer,
                                           size_t size) {
  lfs_soff_t o =
      lfs_file_seek(&real_filesystem, &fat_cache, offset, LFS_SEEK_SET);
  if (o < 0) {
...
static size_t bulk_update_fat(uint32_t start_cluster, size_t size) {
  ...
  size_t read = bulk_update_fat_buffer_read(initial_sector * BUFFER_SIZE,
  ...
  for (unsigned int i = 0; i < num_clusters; i++) {
    ...
    next_cluster =
        (i < num_clusters - 1) ? current_cluster + 1 : END_OF_CLUSTER_CHAIN;
    if (current_cluster & 0x01) {
      buffer[buffer_index] =
          (buffer[buffer_index] & 0x0F) | (next_cluster << 4);
      buffer[buffer_index + 1] = (next_cluster >> 4) & 0xFF;
    } else {
      buffer[buffer_index] = next_cluster & 0xFF;
      buffer[buffer_index + 1] =
          (buffer[buffer_index + 1] & 0xF0) | ((next_cluster >> 8) & 0x0F);
    }
    ...
    if (((current_cluster & 0x01) && buffer_index + 2 >= sector_size) ||
      ...
      size_t write = bulk_update_fat_buffer_write(initial_sector * BUFFER_SIZE,
                                                  buffer, sizeof(buffer));
      if (write == 0)
        return 0;
      ...
      read = bulk_update_fat_buffer_read(initial_sector * BUFFER_SIZE, buffer,
                                         sizeof(buffer));
      if (read == 0)
        return 0;
      ...
      if (current_cluster & 0x01) {
        ...
        buffer[buffer_index] =
            (buffer[buffer_index] & 0xF0) | ((next_cluster >> 8) & 0x0F);
      }
    }
    current_cluster++;
  }
...
static void init_fat(void) {
  uint64_t storage_size = 4096 * 512;
  ...
  int err = lfs_file_open(&real_filesystem, &fat_cache, ".mimic/FAT",
                          LFS_O_RDWR | LFS_O_CREAT);
  assert(err == 0);

  uint8_t head[3] = {0xF8, 0xFF, 0xFF};
  lfs_ssize_t s =
      lfs_file_write(&real_filesystem, &fat_cache, head, sizeof(head));
  if (s != sizeof(head)) {
...
static void set_fat_short_filename(uint8_t *short_filename,
                                   const char *filename) {
...
static void restore_from_short_filename(char *filename,
...
static void restore_from_short_dirname(char *filename,
...
static void create_shortened_short_filename(uint8_t *sfn,
  ...
  ext[3] = '\0';
  to_uppercase(ext);
  snprintf((char *)filename, sizeof(filename), "FIL~%04X%-3s", rand() % 0xFFFF,
           ext);
  memcpy(sfn, filename, FAT_SHORT_NAME_MAX);
}

static void create_shortened_short_filename_dir(uint8_t *sfn,
                                                const char *long_filename) {
  (void)long_filename;
  uint8_t filename[FAT_SHORT_NAME_MAX + 1];
...
static void set_directory_entry(fat_dir_entry_t *dir, const char *name,
                                int cluster) {
...
static void set_file_entry(fat_dir_entry_t *dir, struct lfs_info *info,
                           int cluster) {
...
static void set_long_file_entry(fat_dir_entry_t *dir, uint16_t *lfn_chunk,
                                uint8_t lfn_order, uint8_t check_sum) {
...
static bool save_temporary_file(uint32_t cluster, void *buffer) {
  ...
  snprintf(filename, sizeof(filename), ".mimic/%d/%1d/%1d", thousands, hundreds,
           tens);
  if (fs_mkdir(filename)) {
    return false;
  }

  snprintf(filename, sizeof(filename), ".mimic/%d/%1d/%1d/%04ld", thousands,
           hundreds, tens, cluster);
  lfs_file_t f;
...
static bool delete_temporary_file(uint32_t cluster) {
    ...
    snprintf(filename, sizeof(filename), ".mimic/%d/%1d/%1d/%04ld", thousands,
hundreds,tens, cluster); int err = lfs_remove(&real_filesystem, filename); if
(err != LFS_ERR_OK) { printf("delete_temporary_file: can't lfs_remove '%s'
error=%d\n", filename, err); return false;
    }
    return true;
}
*/
...
append_dir_entry_volume_label(fat_dir_entry_t *entry,
...
static void long_filename_padding(uint16_t *filename, size_t length,
                                  size_t size) {
...
static fat_dir_entry_t *append_dir_entry_directory(fat_dir_entry_t *entry,
                                                   struct lfs_info *finfo,
                                                   uint32_t cluster) {
  ...
  if (strcmp(finfo->name, ".") == 0 || strcmp(finfo->name, "..") == 0) {
    ...
    size_t len = utf8_to_utf16le(filename, sizeof(filename), finfo->name,
                                 strlen(finfo->name));
...
static fat_dir_entry_t *append_dir_entry_file(fat_dir_entry_t *entry,
                                              struct lfs_info *finfo,
                                              uint32_t cluster) {
 ...
 *
 * Recursively traverse the specified base file system directory and update
 * cache and allocation tables.
 */
static int create_dir_entry_cache(const char *path, uint32_t parent_cluster,
    ...
    if (finfo.type == LFS_TYPE_DIR) {
      *allocated_cluster += 1;
      ...
      if (parent_cluster == 0)
        ...
        snprintf(directory_path, sizeof(directory_path), "%s/%s", path,
                 finfo.name);
      directory_path[LFS_NAME_MAX] = '\0';

      err = create_dir_entry_cache((const char *)directory_path,
                                   current_cluster, allocated_cluster);
      if (err < 0) {
...
static uint32_t cluster_size(void) {
  uint64_t storage_size = 512 * 4096;
  return storage_size / (DISK_SECTOR_SIZE * 1);
}

static bool is_boot_sector(uint32_t sector) {
  return sector == FAT_BOOT_SECTOR;
}

static bool is_fat_sector(uint32_t sector) {
  return sector >= FAT_TABLE_SECTOR && sector < FAT_ROOT_SECTOR;
}

static bool is_root_sector(uint32_t sector) {
  return sector == FAT_ROOT_SECTOR;
}

static bool is_data_sector(uint32_t sector) {
  return sector >= FAT_DATA_SECTOR;
}

/*
 * Returns the boot sector of the FAT image when USB requests sector 0
 */
static void read_boot_sector(uint8_t *buffer, uint32_t buffer_size) {
  TRACE("\e[36mRead read_boot_sector()\e[0m\n");

  memcpy(buffer, fat_boot_sector, FAT_BOOT_SECTOR_SIZE);
  memset(buffer + FAT_BOOT_SECTOR_SIZE, 0, buffer_size - FAT_BOOT_SECTOR_SIZE);
  buffer[510] = 0x55;
  buffer[511] = 0xaa;
}
...
static void save_fat_sector(uint32_t request_block, void *buffer,
                            size_t bufsize) {
...
static void read_root_sector(uint8_t *buffer, size_t buffer_size) {
  TRACE("\e[36mRead read_root_sector()\e[0m\n");

  uint8_t *root_file_entry = buffer;

  memcpy(root_file_entry, file_entry, FILE_ENTRY_SIZE);
  strncpy((char *)root_file_entry, disk_label, 11);
  root_file_entry[11] = 0x08; // Volume Label

  root_file_entry += FILE_ENTRY_SIZE;

  memcpy(root_file_entry, file_entry, FILE_ENTRY_SIZE);
  strncpy((char *)root_file_entry, "CART    WSM", 11);
  root_file_entry[11] = 0x20; // File
  root_file_entry[28] = 0x00; // Size
  root_file_entry[29] = 0x00;
  root_file_entry[30] = 0x00;
  root_file_entry[31] = 0x00;

  root_file_entry += FILE_ENTRY_SIZE;

  memcpy(root_file_entry, file_entry, FILE_ENTRY_SIZE);
  strncpy((char *)root_file_entry, "ICON    BMP", 11);
  root_file_entry[11] = 0x20; // File
  root_file_entry[28] = 0x00; // Size
  root_file_entry[29] = 0x00;
  root_file_entry[30] = 0x00;
...
/*
 * Restore the *result_filename of the file_cluster_id file belonging to
 * directory_cluster_id.
 */
static void restore_file_from(char *result_filename,
                              uint32_t directory_cluster_id,
                              uint32_t file_cluster_id) {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      if (memcmp(dir[i].DIR_Name, "..         ", 11) == 0) {
        ...
        if (parent == 0) {
           ...
           * directory is `cluster==0`, but the actual state of the root
           * directory is `cluster==1`, so it needs to be corrected.
           */
      ...
      if ((dir[i].DIR_Attr & 0x0F) == 0x0F) {
        ...
        memcpy(&long_filename[offset * 13 + 0], long_file->LDIR_Name1,
               sizeof(uint16_t) * 5);
        memcpy(&long_filename[offset * 13 + 5], long_file->LDIR_Name2,
               sizeof(uint16_t) * 6);
        memcpy(&long_filename[offset * 13 + 5 + 6], long_file->LDIR_Name3,
               sizeof(uint16_t) * 2);
        continue;
      }
      ...
      if (dir[i].DIR_Attr & 0x10) { // is directory
        if (is_long_filename) {
          utf16le_to_utf8(filename, sizeof(filename), long_filename,
        ...
        if (dir[i].DIR_FstClusLO == target) {
          ...
          snprintf((char *)result, sizeof(result), "%s/%s", filename,
                   child_filename);
 ...
 *
 * Traverse the allocation table in reverse order and return the length of the
 * allocation chain in offset.
 */
 ...
 */
static void restore_directory_from(char *directory,
                                   uint32_t base_directory_cluster_id,
                                   uint32_t directory_cluster_id) {
...
static find_dir_entry_cache_return_t
find_dir_entry_cache(find_dir_entry_cache_result_t *result,
                     uint32_t base_cluster, uint32_t target_cluster) {
  ...
  for (int i = (base_cluster == 1 ? 1 : 2); i < 16; i++) {
    ...
    find_dir_entry_cache_return_t r =
        find_dir_entry_cache(result, entry[i].DIR_FstClusLO, target_cluster);
...
static void create_blank_dir_entry_cache(uint32_t cluster,
                                         uint32_t parent_dir_cluster) {
  fat_dir_entry_t entry[16] = {0};

  set_directory_entry(&entry[0], ".", cluster);
  set_directory_entry(&entry[1], "..",
                      parent_dir_cluster == 1 ? 0 : parent_dir_cluster);
...
void mimic_fat_read(uint8_t lun, uint32_t sector, void *buffer,
  ...
  if (is_fat_sector(sector)) {
    read_fat_sector(sector, buffer, bufsize);
    return;
  }

  if (is_root_sector(sector)) {
    read_root_sector(buffer, bufsize);
    return;
  }

  uint32_t cluster = sector - 0x10;
  size_t offset = 0;
  ...
  find_dir_entry_cache_return_t r =
      find_dir_entry_cache(&result, 1, base_cluster);
  ...
  lfs_soff_t seek = lfs_file_seek(&real_filesystem, &f,
                                  offset * DISK_SECTOR_SIZE, LFS_SEEK_SET);
  if (seek < 0) {
...
static void difference_of_dir_entry(fat_dir_entry_t *orig, fat_dir_entry_t *new,
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if (new[i].DIR_Name[0] == 0xE5) {
      for (int j = 0; j < 16; j++) {
        ...
        if (new[i].DIR_FstClusLO == orig[j].DIR_FstClusLO &&
            ...
            orig[j].DIR_Name[0] != 0xE5 && new[i].DIR_FileSize != 0) {
          // `delete` or `rename`.
        ...
        if (orig[j].DIR_Attr & 0x10 // directory
            ...
            new[i].DIR_FileSize == 0 && orig[j].DIR_Name[0] != 0xE5 && i == j) {
          // `delete` or `rename`.
          memcpy(delete, &orig[j], sizeof(fat_dir_entry_t));
          delete++;
          break;
    ...
    for (int j = 0; j < 16; j++) {
      ...
      if (strncmp((const char *)new[i].DIR_Name, (const char *)orig[j].DIR_Name,
                  11) == 0 &&
          new[i].DIR_FstClusLO == orig[j].DIR_FstClusLO &&
          new[i].DIR_FileSize == orig[j].DIR_FileSize) {
        is_found = true;
      ...
      // rename
      if (i == j && new[i].DIR_FstClusLO == orig[j].DIR_FstClusLO &&
          new[i].DIR_FileSize == orig[j].DIR_FileSize) {
        memcpy(delete, &orig[j], sizeof(fat_dir_entry_t));
        delete++;
        break;
...
/*
 * Update a file or directory from the difference indicated by *src in
 * dir_cluster_id
 *
 * *src is an array of differences created by diff_dir_entry()
 */
static void update_lfs_file_or_directory(fat_dir_entry_t *src,
                                         uint32_t dir_cluster_id) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if ((dir->DIR_Attr & 0x0F) == 0x0F) {
      ...
      int offset = (long_file->LDIR_Ord & 0x0F) - 1;
      memcpy(&long_filename[offset * 13], long_file->LDIR_Name1,
             sizeof(uint16_t) * 5);
      memcpy(&long_filename[offset * 13 + 5], long_file->LDIR_Name2,
             sizeof(uint16_t) * 6);
      memcpy(&long_filename[offset * 13 + 5 + 6], long_file->LDIR_Name3,
    ...
    if (dir->DIR_Attr & 0x10) { // is directory
      ...
      littlefs_write((const char *)filename, dir->DIR_FstClusLO,
                     dir->DIR_FileSize);
      is_long_filename = false;
      continue;
    } else {
...
static void save_file_clusters(uint32_t cluster, const char *filename) {
  ...
  while (next_cluster < 0xFF8) {
    ...
    seek_pos = lfs_file_seek(&real_filesystem, &f, offset * DISK_SECTOR_SIZE,
                             LFS_SEEK_SET);
    if (seek_pos < 0) {
...
static void delete_dir_entry_cache(fat_dir_entry_t *src,
                                   uint32_t dir_cluster_id) {
  char filename[LFS_NAME_MAX + 1];
...
static void update_file_entry(uint32_t cluster, void *buffer, uint32_t bufsize,
  ...
  TRACE(ANSI_RED "lfs_file_open('%s', cluster=%lu, offset=%u)\n" ANSI_CLEAR,
        result->path, cluster, offset);
  lfs_file_t f;
  int err;
  if (offset == 0) {
    err = lfs_file_open(&real_filesystem, &f, result->path,
...
void mimic_fat_write(uint8_t lun, uint32_t request_block, void *buffer,
                     uint32_t bufsize) {
  (void)lun;
  find_dir_entry_cache_result_t result;
  ...
  uint32_t cluster = request_block - 0x10;
  ...
  if (cluster == 1) { // root dir entry
    ...
    difference_of_dir_entry(&orig[0], (fat_dir_entry_t *)buffer, dir_update,
                            dir_delete);
    ...
    if (base_cluster == 0) {
      ...
      find_dir_entry_cache_return_t r =
          find_dir_entry_cache(&result, 1, cluster);
    ...
    if (r == FIND_DIR_ENTRY_CACHE_RESULT_ERROR) {
      TRACE("mimic_fat_write: find_dir_entry_cache(1, base_cluster=%lu) "
            "error=%d\n",
            base_cluster, r);
      return;
    }
