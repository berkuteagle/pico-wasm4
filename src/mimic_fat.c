...
static uint8_t file_entry[] = {
    ...
    0x00, 0x00,                             // UpdateTime
    0x00, 0x00,                             // UpdateDate
...
{
    return sector > FAT_BOOT_SECTOR && sector < FAT_ROOT_SECTOR;
}
 ...
 */
static void read_boot_sector(uint8_t *buffer, uint32_t bufsize)
{
    TRACE("\e[36mRead read_boot_sector()\e[0m\n");

    memcpy(buffer, fat_boot_sector, FAT_BOOT_SECTOR_SIZE);
    memset(buffer + FAT_BOOT_SECTOR_SIZE, 0, bufsize - FAT_BOOT_SECTOR_SIZE);
    buffer[510] = 0x55;
    buffer[511] = 0xaa;
}
